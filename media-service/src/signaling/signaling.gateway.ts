...
export class SignalingGateway
    ...
    handleReaction(
        @MessageBody() data: { roomId: string; peerId: string; emoji: string },
        @ConnectedSocket() client: Socket,
    ) {
        const { roomId, peerId, emoji } = data;
        console.log(`>> received reaction from ${peerId} in room ${roomId}: ${emoji}`);
        // Fan-out to all OTHER peers in the room
        client.to(roomId).emit('reaction', { peerId, emoji });
    }

    @SubscribeMessage('transcript')
    handleTranscript(
        @MessageBody() data: { roomId: string; peerId: string; displayName: string; text: string; lang: string },
        @ConnectedSocket() client: Socket,
    ) {
        const { roomId, peerId, displayName, text, lang } = data;
        // Fan-out to all OTHER peers in the room
        client.to(roomId).emit('transcript', { peerId, displayName, text, lang });
    }
}